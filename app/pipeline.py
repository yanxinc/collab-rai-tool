...
def stakeholders(sys_info):
    ...
    messages.append({'role': 'user', 'content': 
                    "First, identify the most relevant stakeholder(s). Then, out of the list, highlight 1 direct, 1 indirect, and 1 surprising stakeholder that can be impactced by my system's output."})
        ...
        while True:
            rsp = openai.ChatCompletion.create( 
                ...
                messages= [
                    ...
                    {"role": "user", "content": f"{text}\nFirst, write a more concrete and detailed version of the above type of scenario. \nFormat your response as: Only output the shortened scenario."}
                ]
            )
...
def generate_scenarios(st, sys_info, goal, given_stakeholders=None):
    ...
    start = time.time()
    final_scenarios = ranker(scenarios)
