...
st.markdown("User Stories describe the uses of the system from the perspective of an end user. It typically follows the format of: 'As a [role], I want [function], so that [value]'. Please describe 1 primary use case of your system that involves some notion of AI usage. The following sections will be completed based on your user story. \n\n_Note: A complete Responsible AI Impact Assessment often involves multiple user stories with the following sections repeated for each user story. This study is only a subset of the entire assessment and there are more questions, but excluded here._", help="Example User Story:\n\nAs a *product manager*, \n\nI want *an AI-powered feature that can automatically categorize customer feedback into actionable insights*,\n\n so that *we can more efficiently identify and prioritize improvements to our product, enhancing user satisfaction and engagement*.")

st.session_state[f'us1_des'] = st.text_area(f"User Story 1 Description", value=st.session_state.get(f"us1_des", ""))
